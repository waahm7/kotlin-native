...
internal class CodeGeneratorVisitor(val context: Context, val lifetimes: Map<IrElement, Lifetime>) : IrElementVisitorVoid {
    ...
    private inner class FunctionScope(val declaration: IrFunction?, val functionGenerationContext: FunctionGenerationContext) : InnerScopeImpl() {
        ...
        override val exceptionHandler: ExceptionHandler
            get() = ExceptionHandler.Caller(false)

        override val exceptionHandlerExternal: ExceptionHandler
            get() = ExceptionHandler.Caller(context.memoryModel == MemoryModel.EXPERIMENTAL)
    ...
    private inner abstract class CatchingScope : InnerScopeImpl() {
         ...
         * then this exception is passed to [handler]. Do not switches thread state.
         */
         ...
         *
         *  - Returns [landingpad] if the legacy MM is used.
         *  - Returns a separate landingpad BB which switches the thread state to Runnable, if the experimental MM is used.
         */
        private val landingpadExternal: LLVMBasicBlockRef by lazy {
            if (context.memoryModel != MemoryModel.EXPERIMENTAL) {
                return@lazy landingpad
            }

            using(outerContext) {
                functionGenerationContext.basicBlock("landingpad_external", endLocationInfoFromScope()) {
                    genLandingpad(true)
                }
            }
        }
